...
class RobotController:
    def __init__(self):
        ...
        self.angular_rotate_speed = 0.5    #! Speed rotate robot
        self.angular_adjust_right_speed = 0.05
        self.distance_forward = 0.20       #! move forward robot 20 cm (1 step)
    ...
    def lidar_callback(self, data):
        ...
        lidar_indices = lidar_indices.astype(int)  # Chuyển đổi thành kiểu int 
        # self.lidars = data.ranges[lidar_indices] #! error
        lidar_data = [data.ranges[i] for i in lidar_indices]
        self.lidars = np.array(lidar_data)

    def move_forward(self, distance):
        twist_cmd = Twist()
        twist_cmd.linear.x = self.move_forward_speed  #! Forward speed
    ...
    def run(self):
        ...
        print("Angle Current: {} degree".format(self.convert_degree(self.current_yaw_2pi)))
        print("Angle Goal : {} degree".format(self.convert_degree(goal_angle)))
        print("ALPHA : {} degree".format(self.convert_degree(alpha)))
        rospy.sleep(1)
        while not rospy.is_shutdown():
            action = input("Enter action (w: move forward, a: turn left, d: turn right): ")
            if action == 'w':
